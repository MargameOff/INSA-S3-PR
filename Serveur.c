...
void abandon_game(player_t *player);
...
void remove_challenge(player_t *player)
{
    ...
    if (has_challenge)
    {
        // Déterminer l'ordre de verrouillage basé sur les adresses des mutex
        if (&player->player_mutex < &other_player->player_mutex)
        {
            first_mutex = &player->player_mutex;
            second_mutex = &other_player->player_mutex;
        }
        else
        {
            first_mutex = &other_player->player_mutex;
            second_mutex = &player->player_mutex;
        }
...
void abandon_game(player_t *player)
{
    ...
    // Verrouiller le mutex du joueur pour accéder en toute sécurité à game_count et games
    pthread_mutex_lock(&player->player_mutex);
    ...
    // Faire une copie des parties à abandonner
    int game_count = player->game_count;
    game_t *games_to_abandon[MAX_GAMES_PER_PLAYER];
    for (int i = 0; i < game_count; ++i)
    {
        games_to_abandon[i] = player->games[i];
    }

    pthread_mutex_unlock(&player->player_mutex);

    // Parcourir les parties à abandonner
    for (int i = 0; i < game_count; ++i)
    {
        game_t *game = games_to_abandon[i];
        ...
        if (!game->game_over)
        {
            ...
            // Déterminer l'ordre de verrouillage des mutex
            pthread_mutex_t *first_mutex, *second_mutex;
            ...
            // Déverrouiller les mutex des joueurs
            pthread_mutex_unlock(second_mutex);
            pthread_mutex_unlock(first_mutex);
...
void handle_command(player_t *player, char *command)
{
    ...
    else if (strncmp(command, "/play", 5) == 0)
    {
        ...
        while (*cmd_args == ' ') cmd_args++; // Ignorer les espaces
...
// Les autres fonctions nécessaires pour le fonctionnement du serveur
